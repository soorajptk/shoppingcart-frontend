...
//   genaralReducer:{product:prdcts[],productlist:prdcts[]}
// }
// type props={
//   product:prdcts[]
// }
function ViewProduct (pro:any){
  const { id } = useParams<any>();
  const [item, setItem] = useState<any>({});
  const {product}=pro
  useEffect(() => {
    let prdct = product.find((item:any) => item.product_id === Number(id));
    setItem(prdct);
  }, [id,product]);
  return (
...
function mapsStateToProps(state:any){
  return {product:state.genaralReducer.productlist}
}

export default connect(mapsStateToProps) (ViewProduct)