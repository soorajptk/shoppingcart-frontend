...
function Card({ product_name, price,product_id,product}:any) {
  ...
  const addToCart=async(product_id:any,userId:any)=>{
    try {
      const response=await axios({method:'post',url:"http://localhost:5000/api/v1/product/addcart",headers:{"Authorization":"Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6MjcsImlhdCI6MTY1NTI4Mzc2MSwiZXhwIjoxNjU3ODc1NzYxfQ.6ZcpRkFO1kOCp4MGohF9zWIgCHyIqYmN3tz02fl_vlE"},data:{userId,product_id}})  
  ...
  return (
    <div className="card-Container">
      ...
      <div className="card-para">
        ...
        {user &&  <button onClick={()=>addToCart(product_id,user.response.id)}>Add to Cart</button>}
      </div>
    </div>
  );
}
